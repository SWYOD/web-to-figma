...
5. Хост в этих записях — **`localhost`, не `127.0.0.1`**, хотя сам сервер
   (см. выше) слушает строго `127.0.0.1`. Обнаружено при первом импорте
   манифеста в Figma: `"ws://127.0.0.1:52847" must be a valid URL` — валидатор
   Figma принимает домены/`localhost` в `allowedDomains`, но не IP-литералы
   (подтверждено официальной документацией: примеры для локальной разработки
   везде используют `localhost`, IP-адреса не упоминаются вовсе). Плагин
   (`figma-plugin/src/ui/App.tsx`) поэтому обращается к `http://localhost:<port>/pairing`
   и `ws://localhost:<port>`, не к `127.0.0.1` — проверено вживую (и `fetch`,
   и `WebSocket` через `localhost` успешно достучались до сервера, слушающего
   только `127.0.0.1`, на машине разработки), но это резолвится ОС/браузером
   на каждой конкретной машине заново — тот же класс риска, что уже
   вылезал с Vite dev-сервером (`docs/architecture.md`, риск про
   `renderer.server.host`), если когда-нибудь всплывёт на другой машине —
   решение то же: явно проверить, что `localhost` резолвится туда же, куда
   слушает сервер.
